...
if len(sys.argv) != 3:
    print("Use: python3 filter_miRBase_for_bilaterian_families.py <miRBase_mature.fa> <miRBase_hairpin.fa>")
    sys.exit(0)

work_dir = os.getcwd() + "/"
conservation_dir = work_dir + "conservation_bilateria/"
...
bilateria_families = [] # filtered list for families ancestral to Bilateria
...
for family in metazoa_families:
    hasNonB = len(nonbilateria & metazoa_families[family]) > 0
    hasProt = len(protostomia & metazoa_families[family]) > 0
    hasDeut = len(deuterostomia & metazoa_families[family]) > 0

    if (hasProt and hasDeut) or (hasNonB and (hasProt or hasDeut)):
        bilateria_families.append(family)
...
for sequence_name in miRBase_matures:
    ...
    if family_name not in bilateria_families:
        continue
    ...
    elif sequence_name in ["ame-miR-33", "hco-miR-71"]: # cases manually identified via exception below
        strand_name = "-5p"
    elif sequence_name in ["aga-miR-133", "ppy-miR-124", "hme-miR-278", "hme-miR-981"]: # ditto
        strand_name = "-3p"
    else:
