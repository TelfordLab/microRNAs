...
if len(sys.argv) != 3:
    print("Use: python3 filter_miRBase_for_mammalia_families.py <miRBase_mature.fa> <miRBase_hairpin.fa>")
    sys.exit(0)

work_dir = os.getcwd() + "/"
conservation_dir = work_dir + "conservation_mammalia/"
...
mammalia = {"oan", "ete", "cfa", "dno", "ocu", "aja", "bbu", "eca", "efu", "pal", "mdo", "meu", "sha", "age", "lla", "cja", "sbo", "sla", "mml", "mne", "pbi", "pha", "mmr", "dma", "oga", "ggo", "hsa", "ppa", "ppy", "ptr", "ssy", "nle", "lca", "cgr", "cpo", "mmu", "rno", "bta", "chi", "oar", "tch", "ssc"}
model_mammalia = {"hsa", "mmu"}

metazoa_families = dict() # stores all families present in Metazoa
mammalia_families = [] # filtered list for families only present in mammals
...
for family in metazoa_families:
    if len(metazoa_families[family]) > 9 and len(metazoa_families[family] - mammalia) < 1 and len(model_mammalia - metazoa_families[family]) < 1:
        mammalia_families.append(family)
...
for sequence_name in miRBase_matures:
    ...
    if family_name not in mammalia_families:
        continue
    ...
    if sequence_name.endswith("-5p") or sequence_name.endswith("-3p"):
        ...
        elif hairpin_name + "-2" in miRBase_hairpins:
            sequence = str(miRBase_hairpins[hairpin_name + "-2"].seq)
        else:
        ...
        if position < len(sequence) / 2 - 20:
            ...
            strand_name = "3p"
        elif sequence_name in ["mdo-miR-151"]: # cases manually identified via exception below
            strand_name = "5p"
        elif sequence_name in ["oar-miR-127", "ggo-miR-296"]: # ditto
            strand_name = "3p"
